# rm(list=ls())
# source("C:/Users/dongy30/Desktop/Marginal_Sim-master/Z_X_01/Sim1/helper_sim1.R")
...
marginalnew_one_round <- function(seed, m, nobs, states, trates, tlim){
  ...
  dtimes_A <- sort(unique(times$at[is.finite(times$at)]))
  
  ## Exposure Data: Z as outcome, X as cov (delay one interval)
  ...
  #Psi0s_cox$noncum_hazard <- Psi0s_cox$hazard - c(0, head(Psi0s_cox$hazard, -1))
  ...
  #Psi0_cox$noncum_hazard <- Psi0_cox$hazard - c(0, head(Psi0_cox$hazard, -1))
  Psi0_cox <- rbind(c(0.0, 0.0, 0.0), Psi0_cox)
  
  
  
  ###################### Outcome Data: Y as outcome, X, Z as cov (delay one interval) ##########################
  ...
  #   long_Y_ctm$start[idx] <- c(0, dtimes_Y[1:(length(dtimes_Y)-1)])
  ...
  #   # long_Y_ctm$Psis_cox_noncum[idx] <- ifelse(times$at[i] <= t, long_Y_ctm$Psis_cox_cum[idx] - c(0, head(long_Y_ctm$Psis_cox_cum[idx], -1)), 1)
  #   # p <- unique(long_Y_ctm$Psis_cox_noncum[idx])
  #   # long_Y_ctm$Psis_cox_noncum[idx] <- ifelse(long_Y_ctm$Psis_cox_noncum[idx] == 0, p[which(p<1 & p>0)], long_Y_ctm$Psis_cox_noncum[idx])
  #   # long_Y_ctm$spc_cox[idx] <- exp(-long_Y_ctm$Psis_cox_cum[idx])
  #   # 
  #   # # Conditional weights; as formula
  #   # long_Y_ctm$Psi0_cox_cum[idx] <- Psi0_cox$hazard[findInterval(t, Psi0_cox$time)]
  #   # long_Y_ctm$cum_exp_beta_X[idx] <- long_Y_ctm$Psi0_cox_cum[idx] * exp(coef(model_cox_A_ctm_conditional)[1] * long_Y_ctm$Xconf[idx])
  #   # long_Y_ctm$Psi0_cox_noncum_exp_beta_X[idx] <- ifelse(times$at[i] <= t, long_Y_ctm$cum_exp_beta_X[idx] - c(0, head(long_Y_ctm$cum_exp_beta_X[idx], -1)), 1)
  #   # p <- unique(long_Y_ctm$Psi0_cox_noncum_exp_beta_X[idx])
  #   # long_Y_ctm$Psi0_cox_noncum_exp_beta_X[idx] <- ifelse(long_Y_ctm$Psi0_cox_noncum_exp_beta_X[idx] == 0, p[which(p<1 & p>0)], long_Y_ctm$Psi0_cox_noncum_exp_beta_X[idx])
  #   # long_Y_ctm$pc_cox[idx] <- exp(-long_Y_ctm$cum_exp_beta_X[idx])
  #   # # long_Y_ctm$pc_cox[idx] <- exp(-long_Y_ctm$Psi0_cox_cum[idx])
  #   # 
  #   # long_Y_ctm$sw_cox[idx] <- (long_Y_ctm$spc_cox[idx]*long_Y_ctm$Psis_cox_noncum[idx])/
  #   #   (long_Y_ctm$pc_cox[idx]*long_Y_ctm$Psi0_cox_noncum_exp_beta_X[idx])
  #   
  #   
  #   # Cox Exposure Model
  #   # Marginal weights
  ...
  for (i in 1:nobs) {
    ...
    if (sum(idx) > 0) {
      ...
      # casebase$Psis_cox_noncum[idx] <- ifelse(times$at[i] <= t, casebase$Psis_cox_cum[idx] - c(0, head(casebase$Psis_cox_cum[idx], -1)), 1)
      # p <- unique(casebase$Psis_cox_noncum[idx])
      # casebase$Psis_cox_noncum[idx] <- ifelse(casebase$Psis_cox_noncum[idx] == 0, p[which(p<1 & p>0)], casebase$Psis_cox_noncum[idx])
      # casebase$spc_cox[idx] <- exp(-casebase$Psis_cox_cum[idx])
      # 
      # # Conditional weights; as formula
      # casebase$Psi0_cox_cum[idx] <- Psi0_cox$hazard[findInterval(t, Psi0_cox$time)]
      # casebase$cum_exp_beta_X[idx] <- casebase$Psi0_cox_cum[idx] * exp(coef(model_cox_A_ctm_conditional)[1] * casebase$Xconf[idx])
      # casebase$Psi0_cox_noncum_exp_beta_X[idx] <- ifelse(times$at[i] <= t, casebase$cum_exp_beta_X[idx] - c(0, head(casebase$cum_exp_beta_X[idx], -1)), 1)
      # p <- unique(casebase$Psi0_cox_noncum_exp_beta_X[idx])
      # casebase$Psi0_cox_noncum_exp_beta_X[idx] <- ifelse(casebase$Psi0_cox_noncum_exp_beta_X[idx] == 0, p[which(p<1 & p>0)], casebase$Psi0_cox_noncum_exp_beta_X[idx])
      # casebase$pc_cox[idx] <- exp(-casebase$cum_exp_beta_X[idx])
      # # casebase$pc_cox[idx] <- exp(-casebase$Psi0_cox_cum[idx])
      # 
      # casebase$sw_cox[idx] <- (casebase$spc_cox[idx]*casebase$Psis_cox_noncum[idx])/
      #   (casebase$pc_cox[idx]*casebase$Psi0_cox_noncum_exp_beta_X[idx])
  ...
  cbout_poi_ctm <- glm(Yevent ~ timebasis_cb + Aexpo + timebasisa_cb + offset(log(futime)), family=binomial(link=logit), weights=iptw, data=casebase)
  pointest[4] <- coef(cbout_poi_ctm)[4]
  varest[4] <- diag(infjack.glm(cbout_poi_ctm, casebase$idx))[4]
  
  ## TODO: Cox exposure model
  cbout_cox_ctm <- glm(Yevent ~ timebasis_cb + Aexpo + timebasisa_cb + offset(log(futime)), family=binomial(link=logit), weights=sw_cox, data=casebase)
