...
# #### Helper Functions ####
source("C:/Users/dongy30/Desktop/Marginal_Sim-master/Z_10_X_01/Sim2/Marginal_sim2_slm.R") 
...
#outpath <- 'C:/Users/dongy30/Documents/Temp'
outpath <- 'C:/Users/dongy30/Desktop/Results/Scenario2/SLM'
...
numexposure <- 2 # poi_weighted, unweighted
numdatasplit <- 1 # ctm only
m <- numoutcome * numexposure * numdatasplit    # how many columns

# My effects of coef, lambda, transition matrix
input_coef = 1

setwd("C:/Users/dongy30/Desktop/Marginal_Sim-master/Z_10_X_01/Sim2")
###### Per simulation specification of st, ss ######

sim <- function(no) { # sim for each core
  
  #### Helper Functions ####
  source("Marginal_sim2_slm.R") 
  source("helper_sim2_slm.R")
  source("estfun-glm.q")
  ...
  pointest <- matrix(NA, numsim, m) #TODO: (change later) 1 exposure models to compare with, 25 rows, 3 cols (ctm only)
...
parallel::clusterEvalQ(cl, c(library(survival), library(splines), library(dplyr)))
system.time(
...
#### Only CB #######
# pointestall <- pointestall[, 3:4]
# varestall <- varestall[, 3:4]
# coverageall <- coverageall[, 3:4]
# powerall <- powerall[, 3:4]
# m<-2
########### End of SLM result organizaiton #############

results <- cbind(colMeans(pointestall),
                 colMeans(pointestall) - rep(input_coef,m), # Bias # Repeat 0.7 true value 3 times cause currently ctm 3 cols
...
vec <- c('_logi_slm', '_unweighted_slm')
...
round(results, 3)
write.table(round(results, 3), file=file.path(outpath, 'SLM_1000_results_sim2'))
