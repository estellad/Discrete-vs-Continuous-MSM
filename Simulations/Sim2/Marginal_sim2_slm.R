...
# source("C:/Users/dongy30/Desktop/Marginal_Sim-master/Z_10_X_01/Sim2/helper_sim2_slm.R")
# library(survival)
# library(splines)
# library(dplyr)
# #
...
marginalnew_one_round <- function(seed, m, nobs, states, trates, tlim){
  ...
  long_A_slm$m_prob <- ifelse(long_A_slm$Aexpo == 1, 1-long_A_slm$m_prob, long_A_slm$m_prob)
  long_A_slm$c_prob <- ifelse(long_A_slm$Aexpo == 1, 1-long_A_slm$c_prob, long_A_slm$c_prob)
  ...
  long_Y_slm$timea <- as.numeric(!(long_Y_slm$Aexpo)) * ifelse(is.finite(long_Y_slm$at), long_Y_slm$stop - long_Y_slm$at, 0.0)
  # long_Y_slm$futime <- long_Y_slm$stop - long_Y_slm$start
  ...
  for (i in 1:nobs) {
    ...
    if (sum(idx) > 0) {
      ...
      j <- which(casebase$Aexpo[idx] == 0)[1]
      if(is.na(j)){
  ...
  long_Y_slm$notA <- as.numeric(!(long_Y_slm$Aexpo))
  coxout_logi_ctm <- coxph(Surv(start, stop, Yevent) ~ notA + timebasisa_cox + cluster(idx), weight=sw_logi, data=long_Y_slm, control = coxph.control(timefix=FALSE))
  pointest[1] <- coef(coxout_logi_ctm)[1]
  varest[1] <- vcov(coxout_logi_ctm)[1,1]

  coxout_unweight_ctm <- coxph(Surv(start, stop, Yevent) ~ notA + timebasisa_cox + cluster(idx), data=long_Y_slm, control = coxph.control(timefix=FALSE))
  pointest[2] <- coef(coxout_unweight_ctm)[1]
  varest[2] <- vcov(coxout_unweight_ctm)[1,1]
  
  
  ## Case-base Outcome Models
  casebase$notA <- as.numeric(!(casebase$Aexpo))
  cbout_logi_ctm <- glm(Yevent ~ timebasis_cb + notA + timebasisa_cb + offset(log(futime))
                        , family=binomial(link=logit), weights=sw_logi, data=casebase)
  pointest[3] <- coef(cbout_logi_ctm)[4]
  varest[3] <- diag(infjack.glm(cbout_logi_ctm, casebase$idx))[4]
  
  cbout_unweight_ctm <- glm(Yevent ~ timebasis_cb + notA + timebasisa_cb + offset(log(futime))
