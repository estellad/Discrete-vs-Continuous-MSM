...
if (1) {
    outpath <- 'C:/Users/ollis/Dropbox/work/students/estella'
    ...
    plot(tgrid, log(lm1/lm0), type='l', ylim=c(-0.5,1.5), ylab='Log hazard ratio', col='red', xlab='Follow-up years', lwd=2, lty='solid')
    # lines(tgrid, log(lm1/lm0), col='red', lwd=2, lty='solid')

    # lines(tgrid-1, log(lm1/lm0), col='blue', lwd=1)
    abline(h=seq(-0.5,1.5,by=0.5), lty='dotted')
    legend('topright', legend=c('Log hazard ratio (stop at t=2 vs always treat)'), 
           col=c('red'), lwd=2, bg='white', lty=c('solid'))
