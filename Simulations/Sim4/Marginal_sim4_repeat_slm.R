...
# #### Helper Functions ####
source("C:/Users/dongy30/Desktop/Marginal_Sim-master/Z_10_X_01/Sim4/Marginal_sim4_slm.R") 
...
sim <- function(no) { # sim for each core
  ...
  #### Helper Functions ####
  source("Marginal_sim4_slm.R") 
  source("helper_sim4_new_slm.R")
  source("estfun-glm.q")
...
# 
# pointestall2 <- pointestall2[, 5:8]
# varestall2 <- varestall2[, 5:8]
# coverageall2 <- coverageall2[, 5:8]
# powerall2 <- powerall2[, 5:8]
# m<-4
...
results2 <- cbind(colMeans(pointestall2),
                  ...
                  100*(apply(pointestall2, 2, var) + (colMeans(pointestall2) - rep(input_coef2,m))^2), # 100*MSE
...
colnames(results2) <- c('Mean', 'Bias', 'SD', 'Mean SE', '100xMSE', '100xMCE', 'Coverage', 'Power')
round(results2, 3)
